...
export const getAllCar = async (request, response) => {
  try {
    ...
    const cacheKey = `cars:${page}:${limit}:${search}`;
    const cachedCars = await getRedis(cacheKey);
    if (cachedCars) {
      return response.status(200).json(cachedCars);
    }
...
export const updateCar = async (request, response) => {
  try {
    ...
    const updateData = {
      ...
      ...(request.body.brand && { brand: xss(request.body.brand) }),
      ...(request.body.model && { model: xss(request.body.model) }),
      ...(request.body.year && { year: xss(request.body.year) }),
